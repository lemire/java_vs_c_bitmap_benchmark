 ...
 */
static std::vector<EWAHBoolArray<uint32_t>>
create_all_bitmaps(size_t *howmany, uint32_t **numbers, size_t count) {
  if (numbers == NULL)
    return std::vector<EWAHBoolArray<uint32_t>>();
...
int main(int argc, char **argv) {
  ...
  std::vector<EWAHBoolArray<uint32_t>> bitmaps =
      create_all_bitmaps(howmany, numbers, count);
  if (bitmaps.empty())
    return -1;
